...
Application state is where state can be passed to any component without having to prop drill (example: context API or Redux). Component state is when a component holds the state locally. Component state would be good for small projects or a branch of an application tree where the state for that component or components would not make sense outside of that 
